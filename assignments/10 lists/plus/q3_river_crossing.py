...
def main():
    ...
    while everything_is_under_control(
        ...
        man_on_left_side = cross_river(left_side, right_side, man_on_left_side)
        display_state(left_side, right_side, man_on_left_side)

    announce_fate(left_side, right_side, man_on_left_side)


def display_state(left_side, right_side, on_left_side):
...
def everything_is_under_control(left_side, right_side, man_is_on_left_side):
...
def cross_river(left_side, right_side, man_is_on_left_side):
    cargo = get_user_choice(left_side, right_side, man_is_on_left_side)
    if cargo != NOTHING:
...
def get_user_choice(left_side, right_side, man_is_on_left_side):
...
def valid_user_choice(left_side, right_side, man_is_on_left_side, cargo):
    return (
        cargo == NOTHING
...
def transport_cargo(left_side, right_side, from_left_to_right, cargo):
...
def announce_fate(left_side, right_side, man_is_on_left_side):
...
def print_losing_message(left_side, right_side, man_is_on_left_side):
    unsupervised_side = right_side if man_is_on_left_side else left_side
    if WOLF in unsupervised_side and GOAT in unsupervised_side:
