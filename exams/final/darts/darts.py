...
def main():
    ...
    while not game_over:
        game_over, player1_remaining_points = round_for_player(PLAYER1, player1_remaining_points)        
        if not game_over:
            game_over, player2_remaining_points = round_for_player(PLAYER2, player2_remaining_points)        
        

def round_for_player(player, remaining_points):
...
def get_error_free_points(player):
    ...
    while points is None:
        points = get_points_scored(player)
        if points is None:
            print("Invalid input!")
